...
test("timeline draws on scroll and revealed content stays readable on return", async ({ page }) => {
  ...
  await expect.poll(() => steps.nth(3).evaluate((el) => Number(getComputedStyle(el).opacity))).toBeGreaterThan(0.9);
  ...
  await expect.poll(() => steps.last().evaluate((el) => Number(getComputedStyle(el).opacity))).toBeGreaterThan(0.9);
