...
- [x] GSAP y ScrollTrigger centralizados para animación narrativa; microinteracciones ligeras resueltas con CSS.
...
- [x] Sección 02: Sobre mí, con presentación profesional bilingüe y stack tecnológico interactivo.
...
- [x] Revisión y aprobación visual de Preguntas frecuentes.
...
- [x] Transiciones narrativas entre secciones y revisión final de rendimiento.
...
Con el servidor de producción activo, `npm run perf:audit` ejecuta la medición comparativa de carga y recorrido completo. Para otro origen se puede definir `PERF_BASE_URL`.

`next/font` descarga las fuentes durante el build y las sirve localmente. El primer build necesita acceso a Google Fonts.

## Rendimiento

- `src/components/animations/gsap-runtime.ts` registra GSAP y ScrollTrigger una sola vez, agrupa las solicitudes de `refresh()` en un único frame y aplica una configuración común. Las animaciones de secciones inferiores se preparan únicamente cuando se aproximan al viewport y durante tiempo ocioso del navegador.
- Lenis mantiene una sola instancia y comparte el ticker de GSAP. Se detiene cuando la pestaña queda oculta, se destruye al desmontar y no compite con `scroll-behavior` nativo.
- Las transiciones continuas de sección animan `transform`, `opacity` y un blur moderado de 5 px en desktop y 2.5 px en móvil. `filter` y `will-change` solo permanecen activos durante el tramo que realmente anima y se limpian al salir.
- El robot muestra únicamente su loader mientras llega la escena. El runtime, WebGL y Spline comienzan a cargar al interactuar o después de una espera controlada y un periodo ocioso si el Hero continúa visible; nunca se montan en móvil y se pausan fuera del viewport o con la pestaña oculta.
- La galería ambiental del Hero redujo sus nodos repetidos de 105 a 70 y suspende animación y `will-change` fuera de pantalla. El tipo de cambio y Sileo también se cargan cuando sus secciones se aproximan o se usan.
- Framer Motion se eliminó del bundle. Menú, botones y visor usan transiciones CSS; el visor mantiene montado su pie y anima únicamente el plano de la imagen.
- Auditoría local del build de producción a 1440 × 900, recorriendo la página completa en 5 segundos: el JavaScript crítico de la ruta —antes de cargar progresivamente Spline y Sileo— bajó de ~897 KB a ~220 KB transferidos y de ~3.06 MB a ~694 KB decodificados. El heap bajó de ~15.1 MB a ~6.2 MB en la medición sin activar WebGL y se observaron 256 frames frente a 5 inicialmente. CLS se mantuvo prácticamente en cero. Los valores son comparativos del mismo entorno headless, no una promesa de FPS para todos los dispositivos.
...
- GSAP: entrada escalonada del Hero y revelado letra por letra del título de Sobre mí. ScrollTrigger vincula la escala, la opacidad y un desenfoque acotado de la sección anterior al scroll.
- CSS: hover, tap, entrada/salida del menú y transición del visor, sin un segundo motor de animación en React.
- `gsap.matchMedia()` revierte animaciones y listeners al desmontar o cambiar las preferencias. Las fronteras comparten una única utilidad y todos los triggers conservan cleanup.
...
- En móvil se elimina el espacio vertical reservado a la antigua fotografía de fondo. Ambos botones comparten una fila con áreas táctiles de al menos 52 px; ahora usan la tipografía condensada, gruesa y de mayor tamaño de los CTA de Servicios. «Hablemos de tu próximo proyecto» navega a `#services` y «Descargar CV» conserva la descarga directa. La escala usa `svh` para considerar las barras del navegador. El contenido puede crecer de forma natural al ampliar texto, sin bloquear el scroll global.
...
`src/components/sections/about.tsx` presenta el perfil de Gabriel con una composición editorial, títulos de gran escala, tonos oscuros y una iluminación naranja discreta. La columna derecha reúne el retrato, una presentación profesional orientada a clientes y 23 tecnologías reales tomadas de su perfil de GitHub. Cada icono conserva el color de su herramienta, se mantiene recto y a escala completa en reposo, muestra su nombre y responde al hover, foco y toque sin perder accesibilidad; el giro se reserva exclusivamente para el hover con mouse. La matriz entra con el reveal general del bloque para evitar que una transformación individual quede retenida por el scroll. Desde 2400 px, un layout ultra wide limita el ancho útil, amplía la escala editorial, distribuye presentación y descripción en dos columnas y lleva las tecnologías a dos filas compactas; la franja de datos inferior también se concentra para evitar vacíos excesivos. La experiencia se describe de forma general mediante plataformas web, sistemas internos, productos digitales, APIs y datos; no se añaden cifras de impacto ni credenciales no verificadas. Todos los textos traducibles están en `about` dentro de `src/i18n/dictionaries.ts`.
...
- En desktop, el Hero conserva su altura de `100vh` y permanece en posición sticky mientras Sobre mí asciende por delante. La escala baja de 1 a 0.84, la opacidad de 1 a 0.08 y el blur alcanza como máximo 5 px según el avance real del scroll.
- En móvil el Hero también permanece sticky para que Sobre mí ascienda por encima. Conserva su altura de 100svh, la reducción es más sutil (hasta 0.96) y el blur se limita a 2.5 px. El contenido de Sobre mí fluye verticalmente y puede ocupar más de una pantalla.
- El inicio de la transición nunca es negativo: las diferencias entre `100svh` y la altura visible al cambiar las barras del navegador móvil no adelantan el efecto. En reposo, el Hero usa `filter: none`, `transform: none` y opacidad completa; al volver arriba recupera nitidez, escala y presencia.
...
- Composición cinematográfica: en desktop una escena de pantalla completa permanece fijada durante el recorrido. El título ocupa el plano izquierdo, permanece inmóvil y conserva su color y escala; únicamente los capítulos se sustituyen en el derecho con grandes números delineados, órbitas, puntos de luz y una palabra de fondo con parallax. En móvil, la introducción y los siete pasos siguen el flujo natural mediante una línea de tiempo vertical, sin cards ni contenido superpuesto. Cada nodo completado cambia a naranja, usa texto blanco y enciende un halo; al regresar hacia arriba recupera su estado pendiente.
- Coreografía GSAP: la entrada comienza con la microetiqueta, continúa por las tres líneas del título, el símbolo, la introducción, la acción y después revela etiqueta, título, descripción y detalle del primer capítulo. El scroll conduce las transiciones posteriores mediante máscaras, desplazamiento vertical, escala y opacidad; la línea inferior mide el avance y revierte naturalmente al subir.
...
- El cierre claro «Todo empieza con una conversación» ya no aparece directamente: la microetiqueta introduce la escena, el título se revela palabra por palabra mediante máscara y desplazamiento, y después entran el texto secundario y el CTA. Esta secuencia usa la misma utilidad de reveals que Servicios.

Todas las fronteras entre capítulos utilizan `src/components/animations/cinematic-section-transition.ts`. Esta utilidad fija el plano anterior cuando corresponde y aplica los mismos valores base de dirección, profundidad, escala, opacidad, blur moderado, easing y scrub. Hero → Sobre mí, Sobre mí → Proyectos, Proyectos → Proceso, Proceso → Servicios, Servicios → Preguntas, Preguntas → Contacto y Contacto → Footer comparten este sistema. `src/components/animations/cinematic-reveal.ts` unifica el easing y el ritmo de los reveals secuenciales sin imponer una composición idéntica a cada capítulo. Lenis suaviza la rueda del ratón en escritorio y sincroniza su avance con el único ticker de GSAP; en pantallas táctiles y con movimiento reducido se conserva el scroll nativo.
...
- Interacción: en desktop con mouse, el plan bajo el cursor gana presencia tipográfica, extiende una línea naranja y reduce con suavidad la presencia de los otros. La atenuación solo se activa desde 900 px y cuando el dispositivo informa un puntero fino con hover real, evitando que un toque deje los demás planes oscurecidos. En móvil todos los planes mantienen el mismo nivel de opacidad y su estado activo: título y línea naranjas, fondo sutil y CTA destacado. El espacio lateral antes reservado a los iconos flotantes se libera en Sobre mí, Proyectos, Proceso y Servicios; los precios en CRC escalan para no recortarse en pantallas estrechas. Las acciones son botones rectangulares sólidos con texto condensado, grueso y centrado; la flecha permanece anclada a la derecha para no desplazar la etiqueta. Conservan contraste claro, foco visible y áreas táctiles de al menos 58 px. Los numerales 01, 02 y 03 suben detrás del contenido para reforzar su presencia decorativa.
- Transición: Servicios asciende por encima del cierre claro de Proceso mientras ese plano reduce escala, pierde opacidad y se desenfoca de forma moderada. Se usa la transición común y el navbar permanece fuera de los planos animados.
...
La implementación está dividida entre `src/components/sections/services.tsx`, `src/components/ui/service-pricing.tsx`, `src/app/api/exchange-rate/route.ts`, `src/components/animations/services-motion.tsx`, `src/styles/services.css` y el bloque `services` de `src/i18n/dictionaries.ts`. El endpoint interno valida la respuesta de Hacienda y guarda la consulta de origen durante una hora. `tests/services.spec.ts` comprueba contenido ES/EN, conversión USD/CRC con una cotización simulada, indisponibilidad de la fuente, ancho útil móvil, ausencia de atenuación táctil, ajuste completo a 1366 × 768, distribución ultra wide a 2560 × 1200, legibilidad, CTA de WhatsApp, transición compartida, reveal del cierre anterior, movimiento reducido y fallback sin JavaScript. Con el servidor activo, `node scripts/preview-services.mjs` genera las capturas `artifacts/services-*.png`, incluida la variante `ultra-wide`.
...
La transición Servicios → Preguntas reutiliza `createCinematicSectionTransition`; el contenido de Servicios retrocede, se reduce, pierde opacidad y se desenfoca mientras la FAQ asciende por encima. La microetiqueta, el título, el texto y el CTA entran en secuencia; cada pregunta se revela al entrar al viewport. `SmoothScroll` integra Lenis con GSAP en escritorio para un desplazamiento de rueda fluido y ágil. Móvil, dispositivos táctiles y usuarios con movimiento reducido conservan el desplazamiento nativo. `tests/faq.spec.ts` cubre idiomas, acordeón, ancho y posición fija en escritorio.

## Sección 07: Contacto

Disponible en `#contact`, después de Preguntas y desde «Hablemos» en la navegación. La composición divide un panel oscuro de presentación y contacto directo de un formulario claro; en móvil ambos se apilan sin perder ancho útil. El número de WhatsApp confirmado (+506 8344 2305) y el correo ofrecen alternativas directas. El formulario solicita nombre, tipo de proyecto y detalles; el objetivo principal es opcional. Al pulsar la acción, prepara un mensaje localizado en una nueva pestaña de WhatsApp para que el visitante lo revise y lo envíe por sí mismo. El mensaje incorpora la URL pública localizada del portfolio para que WhatsApp pueda mostrar su tarjeta de vista previa; `public/images/portfolio-preview.jpg` proporciona una imagen social optimizada de 1200 × 630 y Twitter utiliza una tarjeta grande. No se guarda información ni se envía desde el portfolio. Si faltan datos obligatorios, [Sileo](https://sileo.aaryan.design/docs) muestra un aviso traducido en lugar del mensaje nativo del navegador, marca los campos pendientes y enfoca el primero. En móvil, el campo enfocado se centra para que el aviso no lo tape. Las flechas de ambas acciones se desplazan al hacer hover o al enfocar con teclado, salvo con movimiento reducido.

Preguntas → Contacto usa la transición cinematográfica común. La introducción, el título, la acción directa y los campos entran en una secuencia GSAP escalonada, con desactivación de efectos para movimiento reducido. La sección está implementada en `src/components/sections/contact.tsx`, `src/components/ui/contact-form.tsx`, `src/components/animations/contact-motion.tsx`, `src/styles/contact.css` y `contactSection` en los diccionarios ES/EN. `tests/contact.spec.ts` comprueba navegación, traducción, ancho móvil, validación, contenido del mensaje y movimiento reducido. Gabriel aprobó la composición de Contacto antes de avanzar al footer.
...
Un cierre legal independiente muestra el año actual, «Todos los derechos reservados», Aviso de privacidad y Términos y condiciones. El regreso al inicio se mantiene únicamente en el grupo de botones flotantes para evitar duplicarlo. Las dos rutas legales están localizadas como `/{locale}/privacy` y `/{locale}/terms`, y se abren mediante navegación cliente del App Router. Contacto → Footer conserva `createCinematicSectionTransition`: Contacto retrocede, pierde presencia y se desenfoca mientras el footer asciende por encima. La secuencia GSAP revela la etiqueta, el nombre, la invitación, las columnas, sus enlaces y el cierre legal en ese orden. Con movimiento reducido, todo permanece visible y en el flujo natural.
...
El navbar permanece fijo por encima de las secciones y fuera del contenedor que reduce escala y opacidad. Su fondo oscuro tiene 30 % de opacidad en el Hero y 60 % al desplazar, con un `backdrop-filter: blur(10px)` moderado y compatibilidad WebKit. La altura es de 56–80 px en desktop (54 px en pantallas bajas) y 64 px en móvil (58 px en pantallas bajas). Los anclajes reservan espacio para la navegación.

`src/components/layout/floating-actions.tsx` muestra una columna de botones circulares monocromáticos al terminar el Hero, incluyendo cuando se llega mediante el menú. Usa exclusivamente los destinos confirmados: GitHub, LinkedIn, correo y WhatsApp. El número de WhatsApp es el contacto confirmado de Costa Rica +506 8344 2305; abrirlo no envía mensajes automáticamente. `src/lib/whatsapp.ts` construye de forma centralizada los nueve enlaces de WhatsApp del sitio: conserva el mensaje específico de Servicios, Preguntas o formulario y añade siempre la URL localizada del portfolio para compartir la misma vista previa social desde Proceso, Servicios, Preguntas, Contacto, footer y el botón flotante. El último botón regresa arriba y devuelve el foco al inicio. Antes de ese punto los controles están ocultos y son `inert`, por lo que no reciben foco. En móvil, los iconos pueden superponerse al contenido para aprovechar todo el ancho disponible.

## Robot del Hero

`src/components/sections/hero-robot.tsx` muestra únicamente «Cargando robot» y carga mediante `React.lazy` y `Suspense` la [escena de Spline proporcionada por Gabriel](https://prod.spline.design/kZDDjO5HuC9GJUM2/scene.splinecode), usando el [componente oficial React Spline](https://github.com/splinetool/react-spline). El runtime y la escena se solicitan inmediatamente al interactuar o, sin interacción, después de doce segundos y un periodo ocioso si el Hero sigue visible. La escena requiere conexión y WebGL; el contenido y las acciones nunca esperan esa descarga.
...
El origen público confirmado es `https://portfolio-gabriel-lemon.vercel.app`. `NEXT_PUBLIC_SITE_URL` permite sustituirlo cuando el portfolio se mueva a un dominio propio y mantiene estables canonical, alternates y el enlace incluido en el mensaje preparado para WhatsApp.

Los títulos, descripciones, textos alternativos y locales Open Graph están traducidos. La vista previa social usa `public/images/portfolio-preview.jpg`, una captura optimizada del Hero en formato 1200 × 630. Twitter usa `summary_large_image`; WhatsApp obtiene de la misma ruta el título, la descripción y la imagen del portfolio.
...
- **2026-09-17 — Contacto y pie de galería:** el pie de cada captura permanece visible y montado mientras se anima únicamente la imagen. Se añade Contacto en `#contact` con composición oscura/clara, acceso directo al número confirmado, formulario bilingüe que prepara el mensaje de WhatsApp, transición compartida desde Preguntas y entrada secuencial. Contacto queda para revisión visual antes de avanzar.

- **Revisión de Contacto:** animación breve de las flechas de las dos acciones; validación de campos obligatorios con avisos de Sileo en español e inglés, resaltado de campos pendientes y foco en el primero. Se evita el mensaje nativo de «campo incompleto».
...
- **Stack integrado en Sobre mí:** la presentación profesional deja de depender del nombre de una institución y explica de forma general el valor del perfil full stack. La columna derecha incorpora JavaScript, TypeScript, HTML, CSS, Tailwind, Bootstrap, React, Next.js, Astro, Svelte, Node.js, Python, Java, PHP, .NET, MongoDB, MySQL, Firebase, Figma, Postman, Git, GitHub y Visual Studio con entrada secuencial y microinteracciones accesibles.
